...
begin
  ...
  try
    ...
    Result.Porta := IniCfg.ReadInteger(SESSAO_SGDB, IDENT_PORTA, 3306);
    Result.Banco := IniCfg.ReadString(SESSAO_SGDB, IDENT_BANCO, 'DBMESTREESCOLA');
    Result.Usuario := IniCfg.ReadString(SESSAO_SGDB, IDENT_USUARIO, 'root');
    Result.Senha := IniCfg.ReadString(SESSAO_SGDB, IDENT_SENHA, 'toor');
